 ...
 * uploaded images, as a .tar.gz. Used by the /api/backup routes (the header
...
// DB_HOST, DB_PORT, DB_USER, DB_PASS, DB_DATABASE, UPLOADS_DIR; process.env fits.
...
/**
 * Replaces the database and the uploads with the archive's. Checks the archive
 * first (NotABackup, nothing touched), then backs up what is here and stops if
 * that fails, then loads the database, then swaps the uploads.
 */
export async function restore(
  ...
  try {
    ...
    try {
      // Database first: if it fails, the uploads still match what is loaded.
      await run(["mysql", ...connArgs(env), db], sql)

      // UPLOADS_DIR is a volume mount in the container: empty it, never remove it.
    ...
    return {
      safety,
      uploads: files.filter((f) => f.isFile()).length,
