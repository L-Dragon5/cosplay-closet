 ...
 *
 * Replaces the database and uploads with the archive's, after backing up what
...
try {
  const { safety, uploads, counts } = await restore(archive)
  console.log(`saved what was here first -> ${safety}`)
  console.log(`restored ${uploads} upload(s)\n\nrows now:`)
